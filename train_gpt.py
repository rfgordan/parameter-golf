...
def serialize_and_validate_roundtrip(
    ...
    metrics: dict[str, float | int | str] = {"quant_name": "int8+zlib"}
    if master_process:
...
def main() -> None:
    ...
    if wandb_run is not None:
        wandb_log(
            {
                "roundtrip/int8_zlib_val_loss": float(roundtrip_metrics["roundtrip_val_loss"]),
                "roundtrip/int8_zlib_val_bpb": float(roundtrip_metrics["roundtrip_val_bpb"]),

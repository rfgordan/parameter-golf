...
# Strong submissions often spend precision on the tied embedding because it
# serves both input and output roles and is unusually quantization-sensitive.
...
def quantize_float_tensor(t: Tensor, quant_range: int = INT8_QUANT_RANGE) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
    ...
    # Vectors / scalars use a simpler full-range int8 scale.
    clip_abs = float(torch.quantile(t32.abs().flatten(), INT8_CLIP_Q).item()) if t32.numel() else 0.0
    scale = torch.tensor(clip_abs / float(INT8_QUANT_RANGE) if clip_abs > 0 else 1.0, dtype=torch.float32)
    q = torch.clamp(torch.round(torch.clamp(t32, -clip_abs, clip_abs) / scale), -INT8_QUANT_RANGE, INT8_QUANT_RANGE).to(torch.int8).contiguous()
    return q, scale
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor]):
    ...
    for name, tensor in state_dict.items():
        ...
        q, s = quantize_float_tensor(t, quant_range=INT6_QUANT_RANGE if USE_INT6 else INT8_QUANT_RANGE)
        if s.ndim > 0:
            qmeta[name] = {
                "scheme": "per_row",
                "axis": 0,
                "quant_range": INT6_QUANT_RANGE if USE_INT6 else INT8_QUANT_RANGE,
            }
        quantized[name] = q
        scales[name] = s
...
def load_model_state_dict(path: str) -> dict[str, Tensor]:
    model_path = Path(path).expanduser()
    if not model_path.is_file():
        raise FileNotFoundError(f"INIT_MODEL_PATH does not exist: {model_path}")
    if model_path.suffix == ".ptz":
        with open(model_path, "rb") as f:
            quant_blob = f.read()
        quant_state = torch.load(io.BytesIO(zlib.decompress(quant_blob)), map_location="cpu")
        if not isinstance(quant_state, dict):
            raise TypeError(f"Expected quantized state dict object in {model_path}, got {type(quant_state).__name__}")
        return dequantize_state_dict_int8(quant_state)
    state_dict = torch.load(model_path, map_location="cpu")
    if not isinstance(state_dict, dict):
        raise TypeError(f"Expected state dict in {model_path}, got {type(state_dict).__name__}")
    return state_dict
...
def serialize_and_validate_roundtrip(
    ...
    if master_process:
        ...
        log0(
            f"Serialized model {quant_name}: {quant_file_bytes} bytes "
    ...
    torch.cuda.synchronize()
    log0(
        f"final_{quant_name.replace('+', '_')}_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} "
...
class GPT(nn.Module):
    ...
    def forward_internal(self, input_ids: Tensor) -> tuple[Tensor, Tensor, Tensor]:
        ...
        consist_idx = self.num_recurrent_loops // 2
...
def main() -> None:
    ...
    if args.init_model_path and not args.eval_only:
        raise ValueError("INIT_MODEL_PATH is only supported with EVAL_ONLY=1")
    if args.init_model_path:
        base_model.load_state_dict(load_model_state_dict(args.init_model_path), strict=True)
    ...
    if args.eval_only:
        ...
        log0(f"step:0/0 val_loss:{val_loss:.4f} val_bpb:{val_bpb:.4f} train_time:0ms step_avg:0.00ms")
        log0(f"eval_only_val_complete eval_time:{eval_ms:.0f}ms")
        log0(
